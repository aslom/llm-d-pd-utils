...
if [ ! -e "$GDR_HOME/lib/libgdrapi.so" ] || [ "$FORCE" = true ]; then
    ...
    echo "Installing gdrcopy to $GDR_HOME..."
    cd "$TEMP_DIR"
    curl -sL https://github.com/NVIDIA/gdrcopy/archive/refs/tags/v2.5.tar.gz -o gdrcopy-v2.5.tar.gz
    tar xzf gdrcopy-v2.5.tar.gz && rm gdrcopy-v2.5.tar.gz
    cd gdrcopy-2.5
...
if ! command -v ucx_info &>/dev/null || [ "$FORCE" = true ]; then
    echo "Installing UCX to $UCX_HOME..."
    cd "$TEMP_DIR"
    curl -sL https://github.com/openucx/ucx/releases/download/v1.18.0/ucx-1.18.0.tar.gz -o ucx-1.18.0.tar.gz
...
if ! command -v nixl_test &>/dev/null || [ "$FORCE" = true ]; then
    echo "Installing NIXL to $NIXL_HOME..."
    cd "$TEMP_DIR"
    curl -sL https://github.com/ai-dynamo/nixl/archive/refs/tags/0.1.1.tar.gz -o nixl-0.1.1.tar.gz
    tar xzf nixl-0.1.1.tar.gz && rm nixl-0.1.1.tar.gz
    cd nixl-0.1.1
